...
App.controller('clientsController', ['$scope', '$http', function($scope, $http) {
    ...
    const urlLoadClients = appUrl + '/clients';
    ...
    // Fonction pour charger la liste des clients (utilisateurs avec le rôle ROLE_USER)
    $scope.loadClients = function () {
        $http.get('/api/users/clients')
            .then(function (res) {
                $scope.listeUsers = res.data;
                console.log("LISTE DES CLIENTS : ", $scope.listeUsers);
            })
            .catch(function (error) {
                console.error("ERREUR DE RECUPERATION DES CLIENTS : ", error);
            });
    };

    

    // Détection automatique selon la page (clients ou autres users)
