...
App.controller('reservationsController', ['$scope', '$http', function($scope, $http) {
    // Chargement des clients (utilisateurs)
    $scope.clients = [];
    $http.get('/api/users?role=USER').then(function(res) {
        $scope.clients = res.data;
    });

    // Chargement des voitures
    $scope.voitures = [];
    $http.get('/api/voitures').then(function(res) {
        $scope.voitures = res.data;
    });
    $scope.reservations = [];
    $scope.reservationCourante = null;

    $scope.loadReservations = function() {
        $http.get('/api/reservations').then(function(res) {
            // Adapter les noms pour l'affichage (clientNom, voitureNom)
            $scope.reservations = res.data.map(function(r) {
                return {
                    ...r,
                    clientNom: r.clientNom || r.clientId, // à adapter selon DTO
                    voitureNom: r.voitureNom || r.vehiculeId // à adapter selon DTO
                };
            });
        });
    };

    $scope.voirReservation = function(reservation) {
        $scope.reservationCourante = reservation;
                        ...
                        validator.validate().then(function (status) {
                            if (status === 'Valid') {
                                ...
                                .then(response => {
                                    ...
                                    angular.element(document.querySelector('[ng-controller="reservationsController"]')).scope().loadreservations();
    ...
    $scope.loadreservations = function () {
        $http.get(appUrl)
            .then(function (res) {
                ...
                console.log("LISTE DES UTILISATEURS : ", $scope.listereservations);
            })
